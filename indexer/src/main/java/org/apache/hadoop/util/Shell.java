...
abstract public class Shell {
  ...
  /**
   * Get the Unix command for setting the maximum virtual memory available to a
   * given child process. This is only relevant when we are forking a process
   * from within the Mapper or the Reducer implementations. Also see Hadoop
   * Pipes and Hadoop Streaming.
   * 
   * It also checks to ensure that we are running on a *nix platform else (e.g.
   * in Cygwin/Windows) it returns <code>null</code>.
   * 
   * @param memoryLimit
   ...
   * Get the Unix command for setting the maximum virtual memory available to a
   * given child process. This is only relevant when we are forking a process
   * from within the Mapper or the Reducer implementations. see also Hadoop
   * Pipes and Streaming.
   * 
   * It also checks to ensure that we are running on a *nix platform else (e.g.
   * in Cygwin/Windows) it returns <code>null</code>.
...
//    if (lastTime + interval > Time.now()) return;
