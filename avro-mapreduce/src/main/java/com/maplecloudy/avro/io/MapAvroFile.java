...
public class MapAvroFile {
  ...
  public static class Writer<K,V> implements java.io.Closeable {
    ...
    public Writer(Configuration conf, FileSystem fs, String dirName,
      ...
      data.setRecordInterval(indexInterval);
      data.setSyncInterval(conf.getInt("io.seqfile.compress.blocksize", 1000000));
      data.sync();
      
      index = new DataFileWriter<Pair<K,Long>>(
          new ReflectDatumWriter<Pair<K,Long>>((Schema)null, ReflectDataEx.get()));
    ...
    public Writer(File dir, Schema keySchema, Schema valueSchema)
        throws IOException {
      this.keySchema = keySchema;
      this.valueSchema = valueSchema;
      comparator = new AvroKeyComparator<K>();
      ...
      data.setCodec(CodecFactory.deflateCodec(DEFAULT_DEFLATE_LEVEL));
      // data.setCodec(CodecFactory.snappyCodec());
      data.create(
          new Pair<K,V>(Pair.getPairSchema(keySchema, valueSchema)).getSchema(),
          dataFile);
      ...
      index.setCodec(CodecFactory.deflateCodec(DEFAULT_DEFLATE_LEVEL));
...
//      System.out.println(pos);
    ...
    private void checkKey(K key) throws IOException {
      ...
      BinaryEncoder encoder = EncoderFactory.get().directBinaryEncoder(outBuf,
          null);
      GenericDatumWriter<K> writer = new ReflectDatumWriter<K>(keySchema);
      GenericDatumReader<K> reader = new ReflectDatumReader<K>(keySchema);
  ...
  public static class Reader<K,V> implements java.io.Closeable {
    ...
    public Reader(FileSystem fs, String dirName, Configuration conf)
      ...
      conf.setInt("dfs.socket.timeout",120000);
      open(fs, dirName, conf);
    }
    
    public Reader(File dirName) throws IOException {
      // this.keySchema = keySchema;
      // this.valueSchema = valueSchema;
      // INDEX_SKIP = conf.getInt("io.map.index.skip", 0);
      open(dirName);
    ...
    protected synchronized void open(File dirName) throws IOException {
      File dataFile = new File(dirName, DATA_FILE_NAME);
      File indexFile = new File(dirName, INDEX_FILE_NAME);
      
      // open the data
      // this.data = new DataFileReader<Pair<K, V>>(new FsInput(dataFile,
      // conf), new ReflectDatumReader<Pair<K, V>>());
      this.data = new DataFileReader<Pair<K,V>>(dataFile,
      ...
      this.index = new DataFileReader<Pair<K,Long>>(indexFile,
          new ReflectDatumReader<Pair<K,Long>>());
      this.indexFile = indexFile.getPath();
    }
    ...
    public void reLoadIndex() throws IOException {
      ...
      try {
        this.index = new DataFileReader<Pair<K,Long>>(new File(indexFile),
            new ReflectDatumReader<Pair<K,Long>>());
        int skip = INDEX_SKIP;
        long position = 0;
        K lastKey = null;
