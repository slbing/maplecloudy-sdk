...
public class AvroUtils {
	...
	protected static void toString(Object datum, StringBuilder buffer, Schema schema) {
		if (datum == null) {
			buffer.append("{null}");
			return;
		}
		if (schema.getType() == Schema.Type.RECORD) {
			...
			for (Field f : schema.getFields()) {
				...
				buffer.append(": ");
				toString(ReflectData.get().getField(datum, f.name(), count), buffer, f.schema());
			...
			buffer.append("\"");
		} else if (schema.getType() == Schema.Type.FIXED || schema.getType() == Schema.Type.BYTES) {
	...
	public static DataFileWriter Create(Configuration conf, Path path, Schema schema) throws IOException {
		DataFileWriter data = new DataFileWriter(new ReflectDatumWriter((Schema) null, ReflectDataEx.get()));
		data.create(schema, path.getFileSystem(conf).create(path, null));
		return data;
	}

	public static DataFileReader Open(Configuration conf, Path path) throws IOException {
		DataFileReader data = new DataFileReader(new FsInput(path, conf),
				new ReflectDatumReader(null, null, ReflectDataEx.get()));
		return data;
	}

	public static long getRecordNum(Configuration conf, String inputPath, boolean isMap) throws Exception {
		return getRecordNum(conf, new Path(inputPath), isMap);
	}

	public static long getRecordNum(Configuration conf, Path inputPath, boolean isMap) throws Exception {
		...
		if (isMap) {
			...
			if (fs.exists(datafile) && fs.exists(indexfile)) {
				readers = new MapAvroFile.Reader[] { new MapAvroFile.Reader(fs, inputPath.toString(), conf) };
			} else
				readers = AvroMapOutputFormat.getReaders(inputPath, conf);
			try {
			...
			if (fs.isFile(inputPath)) {
				...
				readers = AvroPairOutputFormat.getReaders(inputPath, conf);
			try {
