...
public class ReportKeyComparator<T> extends Configured implements
		RawComparator<T> {
	...
	public void setConf(Configuration conf) {
		...
		if (conf != null) {
			...
			try {
				...
				if (inputschema != null)
					schema = Pair
							.getKeySchema(AvroJob.getMapOutputSchema(conf));
			} catch (Exception e) {
				e.printStackTrace();
				schema = null;
			}
				...
				if (ConfigSchemaData.class.isAssignableFrom(new JobConf(conf)
					...
					try {
						schema = ((ConfigSchemaData) new JobConf(conf)
								.getMapOutputKeyClass().newInstance())
								.getSchema(conf);
					} catch (Exception e) {
	...
	public int compare(byte[] b1, int s1, int l1, byte[] b2, int s2, int l2) {
		// if (UnionData.class.getName().equals(schema.getFullName())) {
		// return BinaryData.compareBytes(b1, s1, l1, b2, s2, l2);
		// } else return
