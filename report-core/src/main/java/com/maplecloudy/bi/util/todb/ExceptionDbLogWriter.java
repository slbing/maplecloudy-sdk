...
public class ExceptionDbLogWriter {
	...
	private final static String logTable = "ExceptionTable";
	...
	public ExceptionDbLogWriter(Connection conn) throws SQLException {
		this.conn = conn;
		logWriter = new LogWriter(this.conn, logTable, LogWriter.WRITE_TO_DB);
		tii = logWriter.getDbTableInfo();
	}
	
	public ExceptionDbLogWriter(String db, Connection conn) throws SQLException {
		this.conn = conn;
		logWriter = new LogWriter(this.conn, db+"."+logTable, LogWriter.WRITE_TO_DB);
		tii = logWriter.getDbTableInfo();
	}
