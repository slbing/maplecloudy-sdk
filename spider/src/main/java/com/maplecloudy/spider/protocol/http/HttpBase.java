 ...
 */
package com.maplecloudy.spider.protocol.http;
...
import com.maplecloudy.spider.protocol.ProtocolException;
import com.maplecloudy.spider.protocol.ProtocolOutput;
import com.maplecloudy.spider.protocol.ProtocolStatus;
import com.maplecloudy.spider.protocol.Response;
import com.maplecloudy.spider.util.DeflateUtils;
import com.maplecloudy.spider.util.GZIPUtils;
...
public abstract class HttpBase implements Protocol {
  ...
  protected int maxContent = 8 * 1024 * 1024;
//	protected int maxContent = 64 * 1024;
  ...
  /**
   * The number of seconds the fetcher will delay between successive requests to
   * the same server.
   */
   ...
   * finishes. This way only one thread at a time accesses a host.
   */
...
//		this.maxContent = conf.getInt("http.content.limit", 64 * 1024);
    this.maxContent = conf.getInt("http.content.limit", maxContent);
  ...
  public ProtocolOutput getProtocolOutput(String url, CrawlDatum datum) {
    ...
    try {
      ...
      Content c = new Content(u.toString(),
          (content == null ? EMPTY_CONTENT : content), response.getHeaders());
  ...
  private void unblockAddr(String host, long crawlDelay) {
    synchronized (BLOCKED_ADDR_TO_TIME) {
      ...
      if (addrCount == 1) {
        ...
        BLOCKED_ADDR_TO_TIME.put(host,
            new Long(System.currentTimeMillis() + crawlDelay));
      } else {
  ...
  protected static void main(HttpBase http, String[] args) throws Exception {
    ...
    ProtocolOutput out = http.getProtocolOutput(url, new CrawlDatum());
