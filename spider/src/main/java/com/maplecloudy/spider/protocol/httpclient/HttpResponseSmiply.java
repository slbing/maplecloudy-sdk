 ...
 */
package com.maplecloudy.spider.protocol.httpclient;
...
import com.maplecloudy.spider.metadata.SpellCheckedMetadata;
import com.maplecloudy.spider.protocol.HttpDateFormat;
import com.maplecloudy.spider.protocol.Response;
import com.maplecloudy.spider.protocol.http.HttpBaseSimply;
 ...
 * @author Susam Pal
 */
public class HttpResponseSmiply implements Response {
	 ...
	 */
	HttpResponseSmiply(HttpSimply http, URL url, CrawlDatum datum, boolean followRedirects)
			throws IOException {
		...
		try {
			...
			if (contentLengthString != null) {
				try {
					contentLength = Integer
							.parseInt(contentLengthString.trim());
			...
			if (Http.LOG.isTraceEnabled()) {
				...
				fetchTrace = new StringBuilder("url: " + url
						+ "; status code: " + code + "; bytes received: "
						+ content.length);
			...
			if (content != null) {
				...
				if ("gzip".equals(contentEncoding)
					...
					if (Http.LOG.isTraceEnabled())
						fetchTrace.append("; extracted to " + content.length
								+ " bytes");
	...
	public HashMap<String, java.util.List<String>> getHeaders() {
		return headers;
	}
