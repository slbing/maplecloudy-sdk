...
public class FetcherSmart extends Configured implements Tool {
	...
	public FetcherSmart(Configuration conf)
	{
		this.setConf(conf);
	}
	...
	public static class FetchMapper extends
		...
		public void BlockRecord() throws InterruptedException {
			if (currentValue != null)
				output(currentKey, currentValue,null,
						CrawlDatum.STATUS_FETCH_RETRY);
		}

		private void output(String key, CrawlDatum datum, Content content,int status) throws InterruptedException {
	...
	public void fetch(Path segment, int threads) throws IOException,
		...
		//UnionData.setUnionClass(job.getConfiguration(), CrawlDatum.class,Content.class);
...
//		job.setOutputValueClass( kim.spider.schema.CrawlDatum.class);
	...
	public static class MOReduce extends Reducer<String, UnionData, String, Object> {
		...
		@Override
		protected void reduce(String key, Iterable<UnionData> values, Context context)
				throws IOException, InterruptedException {
