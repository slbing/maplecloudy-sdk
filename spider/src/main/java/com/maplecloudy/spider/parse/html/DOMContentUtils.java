...
package com.maplecloudy.spider.parse.html;
...
import com.maplecloudy.spider.util.NodeWalker;
...
public class DOMContentUtils {
  ...
  public void getOutlinks(URL base, ArrayList<Outlink> outlinks, Node node,
		...
		if (node.getNodeType() == Node.ELEMENT_NODE) {
			...
			if (params != null) {
				// System.out.println("params="+params.attrName+":"+params.elName+":"+params.childLen);
					...
					if (target != null && !noFollow && !post)
						try {
							URL url = base.toString().indexOf(';') <= 0 ? new URL(
									base, target)
									: fixEmbeddedParams(base, target);
							boolean badd = true;
							if (regex != null) {
								Matcher m = regex.matcher(new BasicURLNormalizer().normalize(url.toString()));
								// ֻ��ƥ������ģ����Ž����ռ�
								badd = m.matches();

							}
							if (badd) {
								Outlink newol = new Outlink(url.toString(), linkText
										.toString().trim());
								if(!outlinks.contains(newol))
									outlinks.add(newol);
							}
						} catch (MalformedURLException e) {
						}
