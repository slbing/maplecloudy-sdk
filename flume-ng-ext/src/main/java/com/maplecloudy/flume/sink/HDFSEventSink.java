...
public class HDFSEventSink extends AbstractSink implements Configurable {
  ...
  private static String DIRECTORY_DELIMITER = System
  ...
  // add by sunflower
  private static final int defaultstableTime = 10 * 60 * 1000;
   ...
   */
  private static final long defaultCallTimeout = 1000000000;
  /**
  ...
  private Map<String,BucketWriter> sfWriters;
  private int stableTime = defaultstableTime;
  ...
  public void configure(Context context) {
    ...
    //add the stable time for hdfs output
    stableTime =  context.getInteger("hdfs.stableTime",defaultstableTime);
   ...
   * the specified amount of time in milliseconds. In case of timeout cancel the
   * callable and throw an IOException
   */
   ...
   * events per Transaction. Find the corresponding bucket for the event. Ensure
   * the file is open. Serialize the data and write it to the file on HDFS. <br/>
  ...
  long nextStableColseTime = Long.MIN_VALUE;
  
  public void doStableClose() throws IOException, InterruptedException {
    long cur = System.currentTimeMillis();
    if (cur > nextStableColseTime) {
      
      List<String> rmkeys = Lists.newArrayList();
      for (String key : sfWriters.keySet()) {
        long ptime = sfWriters.get(key).getTimeStamp();
        Calendar date = Calendar.getInstance();
        date.setTimeInMillis(ptime);
        date.set(Calendar.MILLISECOND,0);
        date.set(Calendar.SECOND, 0);
        date.set(Calendar.MINUTE, 0);
        date.add(Calendar.HOUR, 1);
        if (cur - date.getTimeInMillis() > stableTime) {
          rmkeys.add(key);
        }
      }
      // 关闭已经ready的文件
      for (String key : rmkeys) {
        sfWriters.remove(key).close();
      }
      // 并设置数据ready标识
      for (String key : rmkeys) {
        
        Path rp = new Path(key);
        Path readyFlag = new Path(rp.getParent(), ".ready");
        Configuration conf = new Configuration();
        FileSystem hdfs = readyFlag.getFileSystem(conf);
        if (!hdfs.exists(readyFlag)) hdfs.createNewFile(readyFlag);
      }
      Calendar date = Calendar.getInstance();
      date.setTimeInMillis(cur);
      date.set(Calendar.MILLISECOND,0);
      date.set(Calendar.SECOND, 0);
      date.set(Calendar.MINUTE, 0);
      date.add(Calendar.HOUR, 1);
      nextStableColseTime = date.getTimeInMillis() + stableTime;
    }
  }
  ...
  public void start() {
    ...
    this.sfWriters = new HashMap<String,BucketWriter>(maxOpenFiles);
    sinkCounter.start();
    super.start();
  }
  ...
  private static synchronized UserGroupInformation kerberosLogin(
    ...
    if (curUser != null && curUser.getUserName().equals(principal)) {
      LOG.debug("{}: Using existing principal ({}): {}", new Object[] {sink,
          principal, curUser});
   ...
   */
  private void flush(final BucketWriter bucketWriter) throws IOException,
      InterruptedException {
   ...
   */
  private void close(final BucketWriter bucketWriter) throws IOException,
      InterruptedException {
