...
import com.maplecloudy.flume.sink.HDFSEventSink.WriterCallback;
...
class BucketWriter {
  ...
  BucketWriter(long rollInterval, long rollSize, long rollCount,
      long batchSize, Context context, String filePath, String fileName,
    ...
    this(rollInterval, rollSize, rollCount, batchSize, context, filePath,
        fileName, inUsePrefix, inUseSuffix, fileSuffix, codeC, compType,
        writer, timedRollerPool, user, sinkCounter, idleTimeout,
        onIdleCallback, onIdleCallbackPath, null);
  }
  ...
  BucketWriter(long rollInterval, long rollSize, long rollCount,
      ...
      ScheduledExecutorService timedRollerPool, UserGroupInformation user,
      SinkCounter sinkCounter, int idleTimeout, WriterCallback onIdleCallback,
      String onIdleCallbackPath, Map<String,String> head) {
  ...
  private void doOpen() throws IOException, InterruptedException {
    ...
    synchronized (staticLock) {
      ...
      try {
        ...
        String stablePath = filePath.replace(CURRENT, STABLE);
        targetPath = stablePath + DIRECTORY_DELIMITER
            + fullFileName;
        ...
        if (codeC == null) {
          ...
          fileSystem.mkdirs(new Path(stablePath));
          writer.open(bucketPath, head);
        } else {
          // need to get reference to FS before writer does to avoid shutdown
          // hook
          fileSystem = new Path(bucketPath).getFileSystem(config);
          writer.open(bucketPath, codeC, compType, head);
        }
      } catch (Exception ex) {
  ...
  public synchronized void flush() throws IOException, InterruptedException {
    ...
    if (!isBatchComplete()) {
      ...
      if (idleTimeout > 0) {
        // if the future exists and couldn't be cancelled, that would mean it
        // has already run
        // or been cancelled
          ...
          Callable<Void> idleAction = new Callable<Void>() {
            public Void call() throws Exception {
              try {
                ...
                if (onIdleCallback != null) onIdleCallback
                    .run(onIdleCallbackPath);
              } catch (Throwable t) {
                LOG.error("Unexpected error", t);
              }
              return null;
  ...
  public synchronized void append(Event event) throws IOException,
    ...
    try {
      ...
      LOG.warn("Caught IOException writing to HDFSWriter ({}). Closing file ("
          + bucketPath + ") and rethrowing exception.", e.getMessage());
      try {
        close();
  ...
  public String toString() {
    return "[ " + this.getClass().getSimpleName() + " targetPath = "
        + targetPath + ", bucketPath = " + bucketPath + " ]";
  }
  ...
  public long getTimeStamp() {
    try {
      return Long.parseLong(this.head.get("timestamp"));
    } catch (Exception e) {
